...
Status: P1 **hardware-validated** — D1 = **AOA viable, NO sudo** (the keystone transport unknowns are retired). Live 1 MB echo NOT yet green: one device-side accessory-handoff race remains (host claims the interface before Android exposes the accessory to the app). See HARDWARE-FINDINGS.md. P3 Wave B + P5 live pipeline/latency still await hardware.
