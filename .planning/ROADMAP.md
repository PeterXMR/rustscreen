...
| **P4 Decode + Present** (next on critical path) | `AMediaCodec` decode-to-surface onto `ANativeWindow` (thin `ndk-sys` wrapper) | **Phone** to decode/present. Test input is READY: `out.h264` from P3 (push via `adb`); SPS/PPS in-band so the decoder self-configures | **User+phone** |
