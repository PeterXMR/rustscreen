...
**PR-number anchor (updated 2026-06-04):** latest merged = **PR #22** (item 1 complete: host side #21 + Android USB→decoder rendezvous + keep-screen-awake #22). Item 2's cable-free slice is on branch `feat/p2-arrangeable-display` (**PR #23**, open). The GH PR numbers for items 2–11 (**#23–#32**) are *projected* — they assume the items are opened in ladder order with nothing interleaved. GitHub assigns the real number at creation, so if other PRs land between, shift these accordingly. The **Item** column is the stable identifier; **Depends on** references ladder items as "item N" and real GitHub PRs as "#N".
