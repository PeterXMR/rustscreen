...
| 6 | **#24 (next ◀ CURRENT FOCUS)** | **Latency proof + stream tuning** | P5 | **Step 1 (do first):** per-stage + glass-to-glass latency instrumentation (capture→encode→send→decode→present timestamps) — closes the open **P5 criterion #2** (the < 50 ms proof). **Step 2 (only after numbers exist):** tune the *measured* bottleneck — jitter buffer, drop-to-keyframe, adaptive bitrate. Encode is ~10 ms with ~40 ms headroom, so the bottleneck is downstream; measure before tuning. ⚠️ Latency is only meaningful once a *picture* is visible — see Usability backlog U1 (black screen = empty extended desktop) | item 1 |
