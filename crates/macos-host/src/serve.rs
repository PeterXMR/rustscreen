...
pub fn run_host(opts: &HostOpts, stop: &AtomicBool) -> std::io::Result<()> {
    ...
    'reconnect: loop {
        ...
        // bring_up_aoa already sleeps 200ms per wait-for-phone attempt, so the loop never busy-spins;
        // a replug must reconnect with the lowest latency, so we do NOT add reconnect backoff here.
