...
echo "==> 1/2 Building android-client .so (dist) via cargo-ndk"
...
#
# `--profile dist` is the size-optimized, stripped profile used for shipped artifacts
# (the default `release` profile is tuned for speed; see the `[profile.dist]` rationale
# in the workspace Cargo.toml). cargo-ndk copies the resulting .so into jniLibs via -o.
cargo ndk \
    -t arm64-v8a \
    -o "$REPO_ROOT/android/app/src/main/jniLibs" \
    build --profile dist -p android-client --features live-decode
